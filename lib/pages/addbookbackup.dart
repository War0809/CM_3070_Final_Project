...
class _AddBookPageState extends State<AddBookPage> {
  ...
  String? selectedIsbn;
  String? selectedThumbnail; // To store the selected thumbnail
  ...
  final List<XFile>? images = await picker.pickMultiImage(); // Change to pickMultiImage
  ...
  // Initialize a list to hold all recognized lines
  List<String> allRecognizedLines = [];

  // Process each image using Google ML Kit's TextRecognizer
  ...
  for (XFile image in images) {
    ...
    // Debugging: Log recognized text blocks and lines
    print("Debug: Recognized Text Blocks and Lines for ${image.name}:");
    for (TextBlock block in recognizedText.blocks) {
      print("Block text: ${block.text}"); // Debug line: Prints each block
      for (TextLine line in block.lines) {
        print("Line text: ${line.text}");  // Debug line: Prints each recognized line

        // Filter and process the recognized text line
        List<String> filteredWords = _filterRecognizedText(line.text);
        allRecognizedLines.addAll(filteredWords); // Add filtered words to the main list
      }
    }
  }
  ...
  if (allRecognizedLines.isEmpty) {
    print("No text recognized");  // Debug line: Logs if no text is recognized
    ScaffoldMessenger.of(context).showSnackBar(
  ...
  setState(() {
    _indexController.text = uniqueWords.join(', '); // Combine unique recognized lines
  });
...
List<String> _filterRecognizedText(String text) {
              ...
              ? (data['items'] as List)
                  .map((item) => {
                        ...
                        'thumbnail': item['volumeInfo']['imageLinks'] != null
                            ...
                            : null,
                        'industryIdentifiers': item['volumeInfo']
                            ['industryIdentifiers'], // Retrieve ISBN
                      })
                  .toList()
              : [];
        });
  ...
  void _selectBook(Map<String, dynamic> book) {
    ...
    // Fetch the ISBN and thumbnail from the 'industryIdentifiers' and 'thumbnail' fields
    final identifiers = book['industryIdentifiers'] as List<dynamic>?;
    selectedThumbnail = book['thumbnail']; // Set the selected thumbnail

    selectedIsbn = null; // Reset selected ISBN
    ...
    if (response.statusCode == 200) {
      ...
      if (data['totalItems'] > 0) {
        ...
        setState(() {
          ...
          final identifiers = book['industryIdentifiers'] as List<dynamic>?;
          if (identifiers != null) {
            selectedIsbn = identifiers.firstWhere(
              (id) => id['type'] == 'ISBN_13',
          ...
          selectedThumbnail = book['imageLinks'] != null
              ? book['imageLinks']['thumbnail']
              : null; // Get the thumbnail
        });
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Failed to retrieve book data: ${response.reasonPhrase}'),
        ),
      );
    }
  }
  ...
  Future<void> _addBook() async {
    ...
    final response = await Supabase.instance.client.from('books').insert({
      ...
      'user_id': user.id,
      'thumbnail': selectedThumbnail, // Store the selected thumbnail
    }).execute();
  ...
  expands: false, // Fill the available vertical space
